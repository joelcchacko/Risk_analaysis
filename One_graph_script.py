...
for ticker in tickers:
    try:
        ...
        # Plot the forecast (all tickers on the same graph)
        plt.plot(forecast['ds'], forecast['yhat'], label=f'{ticker} Forecast')

    except Exception as e:
        print(f"Error forecasting for {ticker}: {e}")

# Customize the plot for all tickers
plt.title('Forecasted Prices for All Tickers')
plt.xlabel('Date')
